...
def plot_profile_graphs(ps, directory, style="dark", histograms=False, bias_testing_graphs=False):
    ...
    # plot_track_instantaneous_msd(ps, directory=directory, plot_heatmaps=False, plot_tracks_2D=False,
...
def plot_msd_errorbars(
        profiles: List[profile.Profile],
        labels=(),
        directory: str = None,
        profile_colours=None,
        max_dt_prop=0.2,  # Max time duration for MSD analysis, as proportion of longest duration track in each Profile
        xlim: float = None,
        ylim: float = None,
        grid=True):
    """
    ...
    for prof_index, p in enumerate(profiles):
        ...
        lab = labels[prof_index] if labels else 'series_' + str(prof_index)
...
def plot_msd_allT(
        ...
        xlim=None,
        ylim=None,
        max_dt_prop=0.20,  # Max time duration for MSD analysis, as proportion of longest duration track in each Profile
    ):
    """
    :param profiles:
    :param directory:
...
def calculate_instantaneous_msd(track, half_window_width):
    """
    The window is centred on a jump (between two recorded positions, not a position itself). 
    ...
    With a half window width of 4, the first window centre covers to its left positions 1-4 (starting from 1, e.g. Imaris)
    The first window covers to its right positions 6-9 inclusive. 
    ...
    (The following is a link from intuition to implementation in Python; its complicated and this helps)
    'x' refers to the index used in slicing from lists in Python. To fully slice [0, 1, 2] you need list[0:3]
    '@' refers to window centre. 
    14 positions:
    ...
    out. This method, and the plotting in plot_track_instantaneous_msd below work as they should. 
...
def plot_track_instantaneous_msd(profiles, half_window_width=5,  # All possible dts in the window used.
                                 ...
                                 heatmap_abs_time=True,  # If False, use relative time (xlabel=time step). 
                                 # Additional cagetory, top MSD threshold maintained for this many consecutive steps
                                 consecutive_top_threshold_length_upgrade=4,  
    ...
    Heatmaps can only be plotted when all tracks share the same time step. 
    ...
    for ti in list(range(thresh_data.shape[0])):        
        ...
        for k, g in groupby(enumerate(time_step_indices), lambda x: x[0]-x[1]):            
            ...
            if len(subset_indices) > consecutive_top_threshold_length_upgrade:
    ...
    if centre_on_origin:
        for t in samples:   # Calculate the relative position from time zero.
            ...
            for pos in t.positions:
                # Handles missing (e.g. tracking failure) data. Plot, below, handles None data gracefully. 
